...
printf( "static const MLKEM_KEY mlKemTestKey = {\n" );
printf( "\t/* pub */\n\t%d,\n\t{ ", bytesToBits( MLKEM768_PUBLICKEYBYTES ) );
for( i = 0; i < MLKEM768_PUBLICKEYBYTES; i++ )
	{ if( i && !( i % 8 ) ) printf( "\n\t  " );
	printf( ( i != MLKEM768_PUBLICKEYBYTES - 1 ) ? "0x%02X, " : "0x%02X ", 
			mlkemKey->pubKey[ i ] ); }
printf( "},\n\t/* priv */\n\t%d,\n\t{ ", bytesToBits( MLKEM768_SECRETKEYBYTES ) );
for( i = 0; i < MLKEM768_SECRETKEYBYTES; i++ )
	{ if( i && !( i % 8 ) ) printf( "\n\t  " );
	printf( ( i != MLKEM768_SECRETKEYBYTES - 1 ) ? "0x%02X, " : "0x%02X ", 
			mlkemKey->privKey[ i ] ); }
puts( "}\n\t};" );
fflush( stdout );
}
...
static const CURVE25519_KEY ed25519TestKey = {
	/* pub */
	256,
...
static const CURVE25519_KEY x25519TestKey = {
	/* pub */
	256,
...
/* ML-KEM.  
	...
	HELMET: Yes, we're gonna have to go right to ludicrous size.
	SANDURZ: Ludicrous size? Sir, we've never gone that large before.  I 
			 don't know if this code can take it.
	HELMET: What's the matter, Colonel Sandurz, chicken? 
	SANDURZ: Prepare code... prepare code for ludicrous size */

typedef struct {
	const int pubLen; const BYTE pub[ 1184 ];
	const int privLen; const BYTE priv[ 2400 ];
	} MLKEM_KEY;

static const MLKEM_KEY mlKemTestKey = {
	/* pub */
	9472,
	{  },
	/* priv */
	19200,
	{ ,
	  ...
	  0xC4, 0x28, 0x2A, 0xCF, 0x96, 0xF1, 0xBE, 0xA1,
	  0x33, 0x43, 0xEA, 0x38, 0xC7, 0xDA, 0x88, 0x00,
	  0xFA, 0x00, 0x9F, 0x34, 0xB2, 0x5A, 0x00, 0x87,
	  0xF3, 0x0F, 0x71, 0x7D, 0x69, 0x48, 0x10, 0x81,
	  0x11, 0x6D, 0x31, 0xB0, 0x4B, 0x49, 0x57, 0x3B,
	  0xDA, 0xB8, 0x0B, 0x4D, 0xDA, 0x23, 0x30, 0x90,
...
BOOLEAN loadDHContexts( const CRYPT_DEVICE cryptDevice,
	...
	if( !setLabel( *cryptContext, DH_KEY1_LABEL ) )
		{
		free( dhKey );
		cryptDestroyContext( *cryptContext );
		return( FALSE );
	...
	cryptSetComponent( dhKey->y, dlp1024TestKey.y, dlp1024TestKey.yLen );
	status = cryptSetAttributeString( *cryptContext,
	...
	status = cryptCreateContext( decryptContext, CRYPT_UNUSED, CRYPT_ALGO_DH );
	if( cryptStatusError( status ) )
		{
		free( dhKey );
		fprintf( outputStream, "cryptCreateContext() failed with error "
	...
	if( !setLabel( *decryptContext, DH_KEY2_LABEL ) )
		{
		free( dhKey );
		if( isDevice )
			{
	...
	cryptSetComponent( dhKey->q, dlp1024TestKey.q, dlp1024TestKey.qLen );
	cryptSetComponent( dhKey->g, dlp1024TestKey.g, dlp1024TestKey.gLen );
	cryptSetComponent( dhKey->x, dlp1024TestKey.x, dlp1024TestKey.xLen );
	cryptSetComponent( dhKey->y, dlp1024TestKey.y, dlp1024TestKey.yLen );
	status = cryptSetAttributeString( *decryptContext,
									  CRYPT_CTXINFO_KEY_COMPONENTS, dhKey,
									  sizeof( CRYPT_PKCINFO_DLP ) );
...
BOOLEAN loadECDHContexts( const CRYPT_DEVICE cryptDevice,
						  ...
						  CRYPT_CONTEXT *decryptContext )
	{
	return( loadECDSAContextsEx( cryptDevice, cryptContext, decryptContext, 
								 ECDSA_PUBKEY_LABEL, ECDSA_PRIVKEY_LABEL ) );
	}
...
static BOOLEAN load25519Contexts( const CRYPT_DEVICE cryptDevice,
								  CRYPT_CONTEXT *sigCheckContext,
								  CRYPT_CONTEXT *signContext,
								  const BOOLEAN isX25519 )
	{
	CRYPT_PKCINFO_DJB *curve25519Key;
	const CRYPT_ALGO_TYPE cryptAlgo = isX25519 ? CRYPT_ALGO_25519 : \
												 CRYPT_ALGO_ED25519;
	const BOOLEAN isDevice = ( cryptDevice != CRYPT_UNUSED ) ? TRUE : FALSE;
	const CURVE25519_KEY *curve25519KeyData = isX25519 ? &x25519TestKey : \
														 &ed25519TestKey;
	int status;

	/* Allocate room for the public-key components */
	if( ( curve25519Key = ( CRYPT_PKCINFO_DJB * ) malloc( sizeof( CRYPT_PKCINFO_DJB ) ) ) == NULL )
		return( FALSE );
	...
	if( signContext != NULL )
		...
		cryptInitComponents( curve25519Key, CRYPT_KEYTYPE_PRIVATE );
		cryptSetComponent( curve25519Key->priv, curve25519KeyData->priv, curve25519KeyData->privLen );
		cryptSetComponent( curve25519Key->pub, curve25519KeyData->pub, curve25519KeyData->pubLen );
		status = cryptSetAttributeString( *signContext,
									CRYPT_CTXINFO_KEY_COMPONENTS, curve25519Key,
									sizeof( CRYPT_PKCINFO_DJB ) );
		cryptDestroyComponents( curve25519Key );
		if( cryptStatusError( status ) )
			{
			free( curve25519Key );
			cryptDestroyContext( *signContext );
	...
	cryptInitComponents( curve25519Key, CRYPT_KEYTYPE_PUBLIC );
	cryptSetComponent( curve25519Key->pub, curve25519KeyData->pub, curve25519KeyData->pubLen );
	status = cryptSetAttributeString( *sigCheckContext,
...
BOOLEAN loadX25519Contexts( const CRYPT_DEVICE cryptDevice,
							...
							CRYPT_CONTEXT *decryptContext )
	{
	return( load25519Contexts( cryptDevice, cryptContext,
							   decryptContext, TRUE ) );
	}
...
BOOLEAN loadEd25519Contexts( const CRYPT_DEVICE cryptDevice,
							 ...
							 CRYPT_CONTEXT *signContext )
	{
	return( load25519Contexts( cryptDevice, sigCheckContext,
							   signContext, FALSE ) );
	}
...
BOOLEAN loadMLKEMContexts( const CRYPT_DEVICE cryptDevice,
	...
	{
	CRYPT_PKCINFO_PQC *pqcKey;
	const BOOLEAN isDevice = ( cryptDevice != CRYPT_UNUSED ) ? TRUE : FALSE;
	const MLKEM_KEY *mlkemKeyData = &mlKemTestKey;
	int status;

	/* Allocate room for the public-key components */
	if( ( pqcKey = ( CRYPT_PKCINFO_PQC * ) malloc( sizeof( CRYPT_PKCINFO_PQC ) ) ) == NULL )
		return( FALSE );

	/* Create the decapsulation context */
	if( decContext != NULL )
		{
		if( isDevice )
			{
			status = cryptDeviceCreateContext( cryptDevice, decContext,
											   CRYPT_ALGO_MLKEM );
			}
		else
			{
			status = cryptCreateContext( decContext, CRYPT_UNUSED,
										 CRYPT_ALGO_MLKEM );
			}
		if( cryptStatusError( status ) )
			{
			...
			if( status == CRYPT_ERROR_NOTAVAIL )
				{
				/* ML-KEM support isn't always enabled, in which case we tell 
				   the caller to skip the test */
				return( status );
				}
			fprintf( outputStream, "cryptCreateContext() failed with error "
		...
		if( !setLabel( *decContext, MLKEM_PRIVKEY_LABEL ) )
			{
			free( pqcKey );
			cryptDestroyContext( *decContext );
			return( FALSE );
			}
		cryptInitComponents( pqcKey, CRYPT_KEYTYPE_PRIVATE );
		cryptSetComponent( pqcKey->priv, mlkemKeyData->priv, mlkemKeyData->privLen );
		cryptSetComponent( pqcKey->pub, mlkemKeyData->pub, mlkemKeyData->pubLen );
		status = cryptSetAttributeString( *decContext,
									CRYPT_CTXINFO_KEY_COMPONENTS, pqcKey,
									sizeof( CRYPT_PKCINFO_PQC ) );
		cryptDestroyComponents( pqcKey );
		if( cryptStatusError( status ) )
			{
			free( pqcKey );
			cryptDestroyContext( *decContext );
	...
	if( isDevice )
		{
		status = cryptDeviceCreateContext( cryptDevice, encContext,
										   CRYPT_ALGO_MLKEM );
		}
	else
		{
		status = cryptCreateContext( encContext, CRYPT_UNUSED,
									 CRYPT_ALGO_MLKEM );
		}
	if( cryptStatusError( status ) )
		{
	...
	if( !setLabel( *encContext, MLKEM_PUBKEY_LABEL ) )
		{
		free( pqcKey );
		if( decContext != NULL )
			{
	...
	cryptInitComponents( pqcKey, CRYPT_KEYTYPE_PUBLIC );
	cryptSetComponent( pqcKey->pub, mlkemKeyData->pub, mlkemKeyData->pubLen );
	status = cryptSetAttributeString( *encContext,
									  CRYPT_CTXINFO_KEY_COMPONENTS, pqcKey,
									  sizeof( CRYPT_PKCINFO_PQC ) );
	cryptDestroyComponents( pqcKey );
	free( pqcKey );
