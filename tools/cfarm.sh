...
while getopts "h:r:u:" options ; do
	case "${options}" in
		...
		r) DO_RESTART=1 ;
		   ...
		   if [ "$(echo "$HOSTS" | grep -cw "$RESTART_POINT")" -le 0 ] ; then
		   		echo "Error: Invalid restart point $RESTART_POINT" >&2 ;
		   		exit 1 ;
		   fi ;;
		u) USER=${OPTARG} ;;
...
for host in $HOSTS ; do
	...
	case $host in
		...
		'210'|'211'|'215'|'216')
			# Solaris tools don't support -j.  cfarm215 and 216 have a crazy
			# mix of Sun and GNU/LLVM tools so make and ld are Sun, cc is
			# GNU or LLVM.
			NCORES= ;;
...
#		'203') echo "cfarm203: PPC64 big-endian Linux clang." ;;
	...
	# zone.  Note that tools/buildall.sh will check for the newest installed
	# version of the tools and try and select those, so the two path settings
	# for Solaris 10 and 11 below will need to match what the build script is
	# expecting.
	...
	ssh $SSH_ARGS -i "$SSH_PRIVKEYPATH" $SSH_PORTARG_SSH $USER@$HOSTNAME <<EOL
		...
		if [ -d ./CLIB ] ; then
			if [ -n "\$(ls ./CLIB)" ] ; then
				echo "Removing old cryptlib files." ;
				rm -r ./CLIB/* ;
			fi ;
		else
			mkdir ./CLIB ;
		...
		if [ \$(uname -n | cut -f1 -d'.') = "gcc-solaris10" ] ; then
			PATH=/opt/developerstudio12.6/bin:\$PATH ; export PATH ;
		fi ;
